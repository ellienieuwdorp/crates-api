...
public open class ApiClient(
    ...
    private val authentications: kotlin.collections.Map<String, Authentication> by lazy {
        mapOf(
                "ClientID" to ApiKeyAuth("header", "Client-ID"))
    }

    public companion object {
          /**
           * Placeholder base URL. The local Crates.app server binds to a DYNAMIC,
           * ephemeral port that changes every launch, so this value will NOT work
           * as-is. Callers MUST pass the actual `baseUrl` (discovered at runtime)
           * to the [ApiClient] constructor. This constant is retained only because
           * the generated `*Api` classes reference `ApiClient.BASE_URL` as a default.
           */
          public const val BASE_URL: String = "http://localhost:54735/resources"
    ...
    private fun URLBuilder.appendPath(components: kotlin.collections.List<String>): URLBuilder = apply {
        encodedPath = encodedPath.trimEnd('/') + components.joinToString("/", prefix = "/") { it.encodeURLPathPart() }
    }
